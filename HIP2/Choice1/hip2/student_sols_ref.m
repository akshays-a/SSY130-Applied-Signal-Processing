%NO_PFILE
function [funs, student_id] = student_sols_ref()
...
Nsamples = length(noisy_position);

h_euler = [1/dt , -1/dt];
vel.observation = conv(noisy_position, h_euler) *3600;
vel.observation = vel.observation(1:Nsamples) ;
vel.signal = conv(true_position, h_euler) *3600;
vel.signal = vel.signal(1:Nsamples);
t_euler = (0:Nsamples-1)/dt;

vel.fir_obs = conv(noisy_position, h_diff) *3600;
vel.fir_sig = conv(true_position, h_diff) *3600;
